...
const DynamicPolyline = dynamic(() => import('react-leaflet').then(({ Polyline }) => Polyline), {
  ssr: false,
});


// Dynamic imports for components
const DynamicMapContainer = dynamic(() => import('react-leaflet').then(({ MapContainer }) => MapContainer), {
  ssr: false,
});
const DynamicTileLayer = dynamic(() => import('react-leaflet').then(({ TileLayer }) => TileLayer), {
  ssr: false,
});
const DynamicMarker = dynamic(() => import('react-leaflet').then(({ Marker }) => Marker), {
  ssr: false,
});
const DynamicPopup = dynamic(() => import('react-leaflet').then(({ Popup }) => Popup), {
  ssr: false,
});
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-10">
          <div className="place-select-container">
            ...
            <Select
              isMulti
              options={places.map((place) => ({ value: place.id, label: place.place_name }))}
              ...
              <div className="flex flex-col items-center space-y-5 p-10">
                ...
                {tripResponse.trip_details.map((trip, index) => (
                  ...
                  <div key={trip.index} className="relative">
                      {<div className="absolute left-1/2 mb-5 transform -translate-x-1/2 h-full w-1 bg-teal-500"></div>}
